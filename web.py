...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:////Users/MKAVAKLI/Desktop/sonnnn/genelsonproje_arduino_eklendi/yuztanimlama.db'
db = SQLAlchemy(app)
...
@app.route("/kartnumarasi/<string:kartno>")
@login_required
def kartnumarasi(kartno):
    print(f"{kartno} numralı kart okundu")
    todo = Yuz.query.filter_by(kartno=kartno).first()
    if todo:
        ...
        if not todo1:
            ...
            numara = todo.numara
            sinif = todo.sinif
            yeniKayit = Yoklama(ad_soayd=ad_soyad, numara=numara,
                                complete=True, come_date=datetime.now(), sinif=sinif)
            db.session.add(yeniKayit)
            db.session.commit()
            return ("Yoklamaya eklendi")

        else:
            return("zaten yoklamada var.")
    else:
        return("Bu kart sisteme kayıtlı değil")
...
def register():
    ...
    if request.method == "POST" and form.validate():
        ...
        yeniKayit = Kullanici(kullanici_adi=kullanici_adi,
                              kullanici_sifre=kullanici_sifre, isim=name, mail=email)
        db.session.add(yeniKayit)
        db.session.commit()
...
def cam1(id):
    if id == "1":
        ...
        ard=""
        try:            
            ard = Serial(port="/dev/ttyUSB0", baudrate=9600)
            while True:
                # if ard.inWaiting()!=0:
                deger = str(ard.readline())
                deger = deger.decode("utf-8")
                print(deger)
                if deger:
                    break
                # time.sleep(1)
            ...
            if ard:
                ard.close()
                print("kapandı")       
        deger = deger.split("*")[1].split("*")[0]
        siniflariekle = Sinif.query.all()
        print(deger)
        return render_template('cam1.html', id=id, siniflar=siniflariekle,kartno=deger)
    elif id == "4":
        Egitme.egitme_basla()
        return render_template('cam1.html', id=id)
    elif id == "5":
...
class VideoCameraYT(object):
    ...
    def get_frame(self):
        ...
        for (x, y, w, h) in face_rects:
            ...
            cv2.putText(image, str(tahminEdilenKisi), (x, y + h),
            ...
            if todo:
                ...
                # veritabanına kayıt edilecek.
                    ...
                    if fark >= 1:
                        ...
                        sinif = todo.sinif
                        yeniKayit = Yoklama(
                            ad_soayd=ad_soyad, numara=numara, complete=True, come_date=datetime.now(), sinif=sinif)
                        db.session.add(yeniKayit)
                        db.session.commit()
...
def addYuz():
    ...
    kartno = request.form.get("kartno")
...
def yoklama():
    ...
    bugun = datetime.now()
    return render_template("yoklama.html", yoklama=yoklama,tarih=bugun)
...
#Yoklama Detay İş ve İşlemleri
@app.route("/detay/<string:numara>")
@login_required
def detayogrenci(numara):
    #Gönderilen id göre tablodaki kayıtı SQLAlchemy sorgusu ile alma
    ogrenci1= Yoklama.query.filter_by(numara = numara).first()
    ogrenci = Yoklama.query.filter_by(numara = numara).all()
    
    return render_template("ogrenci.html",ogrencix=ogrenci,ad=ogrenci1.ad_soayd)
#Sınıf Listeleri
@app.route("/siniflistesi")
@login_required
def siniflistesi():
    siniflar = Sinif.query.all()  # Yoklamam tablosundaki tüm kayıtları al
   
    return render_template("siniflistesi.html", sinifx=siniflar)

#Sınıf Detay İş ve İşlemleri
@app.route("/listelex/<string:sinifx>")
@login_required
def detaysiniflar(sinifx):
    #Gönderilen id göre tablodaki kayıtı SQLAlchemy sorgusu ile alma
    
    ogrenci = Yuz.query.filter(Yuz.sinif==sinifx).all()
    
    return render_template("siniflistesix.html",ogrencilist=ogrenci,sinifim=sinifx)
